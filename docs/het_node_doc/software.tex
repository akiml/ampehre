...
\subsubsection{Measuring library usage}\label{sec:libmeasureUsage}
...
\end{lstlisting}
At the beginning the libmeasure has to be initialized. Therefore the \texttt{ms\_init} function is called. \added[id=ck]{The \texttt{ms\_init} function has several parameters which are described in the following paragraph.} The first parameter is a \texttt{MS\_VERSION} struct as defined in the \texttt{ms\_version.h} header, to make sure that the correct version of the libmeasure is installed and used. \added[id=ck]{The next four parameters are used to set the CPU governor as available in the Linux kernel, CPU maximum and minimum frequencies and the GPU frequency. The next parameter defines the timeout settings for the IPMI request in our kernel module. Both possible settings \texttt{IOC\_SET\_IPMI\_TIMEOUT} and  \texttt{IOC\_SET\_AND\_LOCK\_IPMI\_TIMEOUT} set the timeout for an IPMI request via an ioctl call to the system sampling rate minus 10ms or if this value is larger than 200ms to the maximum value of 200ms. The difference is that \texttt{IOC\_SET\_AND\_LOCK\_IPMI\_TIMEOUT} sets the new IPMI timeout and afterwards locks the timeout for calls to \texttt{IOC\_SET\_IPMI\_TIMEOUT}. Only further calls to \texttt{IOC\_SET\_AND\_LOCK\_IPMI\_TIMEOUT} or after the kernel module is closed by every application can change the timeout. Therefore this parameter is used to make sure that the hettime tool is not allowed to change the IPMI timeout while msmonitor is running. Usually it is sufficient to use \texttt{IOC\_SET\_IPMI\_TIMEOUT}, only in the case you are planning to run multiple applications which use the libmeasure in parallel, it should be defined via \texttt{IOC\_SET\_AND\_LOCK\_IPMI\_TIMEOUT} which application sets the IPMI timeout.\\
The second last parameter specifies whether certain measurements should be skipped and only sampled every 10th measurement to reduce the CPU utilization. We have performed extensive profiling to select the measurements which need to be skipped to reduce the CPU utilization. We assume that in most cases e.g. temperature measurements are not as important as for example the power consumption and that it is unlikely that the temperature changes rapidly. Because of that it is not always necessary to perform temperature and power measurements with the same sampling rate. Instead it might be more critical to reduce the influence of the measuring library on the system. The parameter can be set to \texttt{LOW} to perform every measurement at every sampling point or to \texttt{HIGH} to skip some measurements which are shown in detail in the following table.}
\begin{center}
...
\end{tabularx}
\end{center}
\added[id=ck]{If any of these measurement values are needed with the maximum accuracy the \texttt{LOW} option should be used. Otherwise the \texttt{HIGH} option can be used to reduce the CPU utilization caused by the measuring system itself.\\
The last parameter of the \texttt{ms\_init} function specifies which variant of the measuring library is used. There are two variants \texttt{LIGHT} and \texttt{FULL} available. The \texttt{FULL} variant provides the full functionality of the measuring library where all measurements are performed. The \texttt{LIGHT} variant performs not all measurements to reduce the CPU utilization. In comparison to the \texttt{HIGH} option which only skips certain measurements but still performs every measurement periodically, in the \texttt{LIGHT} variant some measurements are not performed at all and thus for this measurements no values can be obtained. The following table shows the difference between the \texttt{LIGHT} and \texttt{FULL} variant, "\checkmark/\checkmark" means available in both variants, "\checkmark/x" only in the \texttt{FULL} variant and "x/x" not available for this resource in both variants.  }
\begin{center}
