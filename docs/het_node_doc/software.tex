...
MSYSTEM *ms = ms_init(&version, CPU_GOVERNOR_ONDEMAND,
                      ...
                      IOC_SET_IPMI_TIMEOUT, SKIP_PERIODIC,
                      VARIANT_FULL);
...
public:
	...
	\begin{lstlisting}[caption={Abstract minimal example for the \texttt{measure()} method of the \texttt{CMeasureResource} class to show the usage of the templates.}, label=lst:CMeasureResourceMeasure]
template <int TSkipMs, int TVariant>
void CMeasureResource<TSkipMs, TVariant>::measure(MEASUREMENT* pMeasurement, int32_t& rThreadNum) {
...
void CMeasureResourceThread::run(void) {
	...
	while (!mThreadStateStop) {
		 ...
		 */
		if(!(skip_ms_cnt++ %
			 mpMeasurement->check_for_exit_interrupts)){
...
Listing \ref{lst:CMgmtExtension} shows an example what a programmer needs add to the constructor in order to implement a new module. In this example we pass the GPU frequency settings to the \texttt{CMeasureResource}.
...
case RESOURCE:
	divide_sampling_rates(&(measurement->resource_time_wait),
							sec, nsec,
							check_for_exit_interrupts);
	measurement->msr_check_for_exit_interrupts =
		check_for_exit_interrupts;
	break;
\end{lstlisting}
...
The third function which has to be modified is \texttt{ms\_alloc\_measurement()}. You should set the \texttt{struct timespec} which holds the sampling rate to maximum and the \texttt{resource\_check\_for\_exit\_interrupts} to the default value 1 (\texttt{SKIP\_NEVER}).
...
measurement->resource_check_for_exit_interrupts	= 1;
\end{lstlisting}
\end{description}
