...
namespace NLibMeasure {
	...
	CMeasureMSRThread<Variant>::CMeasureMSRThread(CLogger& rLogger, CSemaphore& rStartSem, void *pMsMeasurement, CMeasureAbstractResource& rMeasureRes) :
	...
	void CMeasureMSRThread<Variant>::run(void) {
		...
		mrLog() << ">>> 'msr thread' (thread #" << mThreadNum << "): init" << std::endl
				<< "     effective sampling rate: " << mTimer.getTimerHertz() / pMsMeasurementCpu->msr_skip_ms_rate << " Hz / "
				<< mTimer.getTimerMillisecond() * pMsMeasurementCpu->msr_skip_ms_rate << " ms" << std::endl;
		mrLog.unlock();
		...
		while (!mThreadStateStop) {
			...
			if(!(skip_ms_cnt++ % pMsMeasurementCpu->msr_skip_ms_rate)){
				mrMeasureResource.measure(mpMsMeasurement, mThreadNum);
			}
