...
void ms_set_timer(MS_LIST *ms_list, int flag, uint64_t sec, uint64_t nsec, uint32_t skip_ms_rate) {
	if(skip_ms_rate <= 0){
		skip_ms_rate = 1;
	}
	...
	switch(flag) {
		case CPU:
			...
			if(ms_measurement!=NULL) {
				((MS_MEASUREMENT_CPU *)ms_measurement)->msr_time_wait.tv_sec	= sec;
				((MS_MEASUREMENT_CPU *)ms_measurement)->msr_time_wait.tv_nsec	= nsec;
				((MS_MEASUREMENT_CPU *)ms_measurement)->msr_skip_ms_rate		= skip_ms_rate;
			}
			break;
		case GPU:
			ms_measurement = getMeasurement(&ms_list, GPU);
			if(ms_measurement!=NULL) {
				((MS_MEASUREMENT_GPU *)ms_measurement)->nvml_time_wait.tv_sec	= sec;
				((MS_MEASUREMENT_GPU *)ms_measurement)->nvml_time_wait.tv_nsec	= nsec;
				((MS_MEASUREMENT_GPU *)ms_measurement)->nvml_skip_ms_rate		= skip_ms_rate;
			}
			break;
		case FPGA:
			ms_measurement = getMeasurement(&ms_list, FPGA);
			if(ms_measurement!=NULL) {
				((MS_MEASUREMENT_FPGA *)ms_measurement)->maxeler_time_wait.tv_sec	= sec;
				((MS_MEASUREMENT_FPGA *)ms_measurement)->maxeler_time_wait.tv_nsec	= nsec;
				((MS_MEASUREMENT_FPGA *)ms_measurement)->maxeler_skip_ms_rate		= skip_ms_rate;
			}
			break;
		case MIC:
			ms_measurement = getMeasurement(&ms_list, MIC);
			if(ms_measurement!=NULL) {
				((MS_MEASUREMENT_MIC *)ms_measurement)->mic_time_wait.tv_sec	= sec;
				((MS_MEASUREMENT_MIC *)ms_measurement)->mic_time_wait.tv_nsec	= nsec;
				((MS_MEASUREMENT_MIC *)ms_measurement)->mic_skip_ms_rate		= skip_ms_rate;
			}
			break;
		case SYSTEM:
			ms_measurement = getMeasurement(&ms_list, SYSTEM);
			if(ms_measurement!=NULL) {
				((MS_MEASUREMENT_SYS *)ms_measurement)->ipmi_time_wait.tv_sec	= sec;
				((MS_MEASUREMENT_SYS *)ms_measurement)->ipmi_time_wait.tv_nsec	= nsec;
				((MS_MEASUREMENT_SYS *)ms_measurement)->ipmi_skip_ms_rate		= skip_ms_rate;
			}
			break;
		default:
