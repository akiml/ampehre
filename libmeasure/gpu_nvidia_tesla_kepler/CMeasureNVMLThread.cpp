...
namespace NLibMeasure {
	...
	CMeasureNVMLThread<Variant>::CMeasureNVMLThread(CLogger& rLogger, CSemaphore& rStartSem, void* pMsMeasurement, CMeasureAbstractResource& rMeasureRes) :
	...
	void CMeasureNVMLThread<Variant>::run(void) {
		...
		mrLog() << ">>> 'nvml thread' (thread #" << mThreadNum << "): init" << std::endl
				<< "     effective sampling rate: " << mTimer.getTimerHertz() / pMsMeasurementGpu->nvml_skip_ms_rate << " Hz / "
				<< mTimer.getTimerMillisecond() * pMsMeasurementGpu->nvml_skip_ms_rate << " ms" << std::endl;
		mrLog.unlock();
		...
		while (!mThreadStateStop) {
			...
			if(!(skip_ms_cnt++ % pMsMeasurementGpu->nvml_skip_ms_rate)){
				mrMeasureResource.measure(mpMsMeasurement, mThreadNum);
			}
