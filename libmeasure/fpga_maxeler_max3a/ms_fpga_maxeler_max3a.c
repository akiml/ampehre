...
#define MEASUREMENT_FPGA \
	(NULL == getMeasurement(&ms_list, FPGA)) ? 0 : ((MS_MEASUREMENT_FPGA *) getMeasurement(&ms_list, FPGA))

double fpga_time_total(MS_LIST *ms_list) {	
	return MEASUREMENT_FPGA->maxeler_time_runtime;
}

double fpga_energy_total_vcc1v0_core(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[VCC10];
}

double fpga_energy_total_vcc1v5_ddr(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[VCC15];
}

double fpga_energy_total_vcc2v5_aux(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[VCC25];
}

double fpga_energy_total_imgt_1v0(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[IMGT10];
}

double fpga_energy_total_imgt_1v2(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[IMGT12];
}

double fpga_energy_total_mgt_1v0(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[MGT10];
}

double fpga_energy_total_mgt_1v2(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[MGT12];
}

double fpga_energy_total_power_usage(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_energy_acc[POWER];
}

double fpga_power_avg_vcc1v0_core(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[VCC10];
}

double fpga_power_avg_vcc1v5_ddr(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[VCC15];
}

double fpga_power_avg_vcc2v5_aux(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[VCC25];
}

double fpga_power_avg_imgt_1v0(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[IMGT10];
}

double fpga_power_avg_imgt_1v2(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[IMGT12];
}

double fpga_power_avg_mgt_1v0(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[MGT10];
}

double fpga_power_avg_mgt_1v2(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[MGT12];
}

double fpga_power_avg_power_usage(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_power_avg[POWER];
}

double fpga_temp_max_ifpga(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_temperature_max[ITEMP];
}

double fpga_temp_max_mfpga(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_temperature_max[MTEMP];
}

double fpga_util_avg_comp(MS_LIST *ms_list) {
	return MEASUREMENT_FPGA->maxeler_util_comp_avg;
}
