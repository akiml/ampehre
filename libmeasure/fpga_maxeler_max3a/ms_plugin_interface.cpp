...
extern "C" {
	...
	void* init_resource_thread(void* pLogger, void* pStartSem, MS_LIST* pMs_List, void* pMeasureRes){
		...
		if(pMaxeler->getVariant() == FULL) {
			pMaxelerThread =  new NLibMeasure::CMeasureMaxelerThread<FULL>(*((NLibMeasure::CLogger*)pLogger), *((NLibMeasure::CSemaphore*)pStartSem), getMeasurement(&pMs_List, FPGA), *pMaxeler);
		} else {
			pMaxelerThread =  new NLibMeasure::CMeasureMaxelerThread<LIGHT>(*((NLibMeasure::CLogger*)pLogger), *((NLibMeasure::CSemaphore*)pStartSem), getMeasurement(&pMs_List, FPGA), *pMaxeler);
		}
