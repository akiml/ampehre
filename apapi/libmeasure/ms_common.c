...
int __create_mapper(int msId, void *measurement, char *cmp, struct apapi_eventset *set, struct __mapper **result_map){
	...
	for (msIx = 0; msIx < (log2(ALL+1)); ++msIx) {
		...
		for (cmpIx = 0; cmpIx < mgmt->num_cmp; ++cmpIx) {
			if (component_mapping[cmpIx] == msId) {
				...
				__create_mapper(msId, measurement, known_components[cmpIx], mgmt->sets[cmpIx], &(mgmt->update_args[cmpIx].map));
